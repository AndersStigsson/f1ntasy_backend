...
router.get('/all', async (req: Request, res: Response) => {
    ...
    for(var result of results) {
        ...
        userPoints[result.username] = {totalPoints: 0, correctPos: 0, correctTop5:0, totalRaces: 0}
    }
    for(var k = 1; k < races+1; k++){ 
        const tempvalue = await fetch(`http://ergast.com/api/f1/2021/${k}/results.json`);
        ...
        // for(var i = 0; i < tempjson.MRData.RaceTable.Races.length ; i++) {
            ...
            if(tempjson.MRData.RaceTable.Races[0] !== undefined) {
                ...
                resultArray[k-1] = [];
                for(var j = 0; j < 5; j++){
                    resultArray[k-1][j] = tempjson.MRData.RaceTable.Races[0].Results[j].number;
                }
            }
    ...
    for(var user in userPoints) {
        values.push({"name": user, "totalPoints" : userPoints[user].totalPoints, "correctPos": userPoints[user].correctPos, "correctTop5": userPoints[user].correctTop5, "totalRaces": userPoints[user].totalRaces});
    }
    userPoints = values;
    ...
    return res.status(OK).json({userPoints : userPoints, awaitingResults : awaitingResults});
});
